...
当前版本在集成过程中已经改动，不是原版本
*/
; (function ($) {
  $.fn.easyUpload = function (opts) {
    var defaults = {
      allowFileTypes: '*.pdf;*.doc;*.docx;*.jpg,*.png',//允许上传文件类型，格式'*.pdf;*.doc;'
      allowFileSize: 100000,
      selectText: '选择文件',//上传按钮文案
      multi: true,//是否允许多文件上传
      multiNum: 5,//多文件上传时允许的有效文件数
      showNote: true,//是否展示文件上传说明
      note: '提示：最多上传5个文件，超出默认前5个，支持格式为：doc、docx、pdf、jpg、png',//文件上传说明
      showPreview: true,
      url: '',//上传文件地址
    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += '<span class="btn btn-default easy_upload_select noselect">' + option.selectText + '</span>';
          $html += option.showNote ? '<span class="easy_upload_note">' + option.note + '</span>' : '';
          $html += option.multi ? '<i class="fa noselect head_check" data-checked="no">&#xf096;</i>' : '';
          $html += option.multi ? '<span class="btn btn-primary btn-sm easy_upload_head_btn1 noselect"><i class="fa fa-upload"></i> 批量上传</span>' : '';
          $html += option.multi ? '<span class="btn btn-danger btn-sm easy_upload_head_btn2 noselect"><i class="fa fa-trash-o"></i> 批量删除</span>' : '';
          $html += '</div>';
          $html += '<ul class="easy_upload_queue clearfix"></ul>';
          $html += '</div>';
          _ele.html($html);
          this.bindHead();
        },
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            if (fileType == 'bmp' || fileType == 'jpg' || fileType == 'jpeg' || fileType == 'png' || fileType == 'gif') {
              ...
              preview = '<div class="easy_upload_img" style="background-image: url(' + imgSrc + ');"></div>';
            } else if (fileType == 'rar' || fileType == 'zip' || fileType == 'arj' || fileType == 'doc'  || fileType == 'docx' || fileType == 'pdf' || fileType == 'z') {
              preview = '<i class="fa easy_upload_icon">&#xf15b;</i>';
            } else {
              preview = '<i class="fa easy_upload_icon">&#xf15b;</i>';
            }
            var sHtml = '';
            sHtml += '<p class="status status1">可上传</p>';
            sHtml += '<p class="status status2">待上传</p>';
            ...
            $html += option.showPreview ? '<div class="easy_upload_preview queue_item-section">' + preview + '' : '';
            ...
            $html += file.allow ? '<p class="easy_upload_upbtn fa noselect">&#xf093;</p>' : '';
            $html += '<p class="easy_upload_delbtn fa noselect">&#xf014;</p>';
            $html += '</div>';
            $html += '<div class="easy_upload_checkone queue_item-section">';
            $html += option.multi ? '<i class="fa noselect queue_check queue_check_allow_'+ file.allow +'" data-checked="no" data-up="1">&#xf096;</i>' : '';
            $html += '</div>';
            if (option.multi) {
              $(queueUl).append($html);
            } else {
        ...
        _handleFailed: function(param) {
          clearInterval(showTimer);
          $(param.upBar).css("background-color", "#ed4e2a");
